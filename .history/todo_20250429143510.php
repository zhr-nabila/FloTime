...
$deadline = isset($_GET['deadline']) ? trim($_GET['deadline']) : '';

// Build SQL secara dinamis
$whereClauses = ["user_id = ?"];
$params = [$user_id];
$paramTypes = "i"; // 'i' untuk integer user_id

if (!empty($search)) {
    $whereClauses[] = "title LIKE ?";
    $params[] = "%" . $search . "%";
    $paramTypes .= "s"; // 's' untuk string
}
...
if (!empty($deadline)) {
    $whereClauses[] = "DATE(deadline) = ?";
    $params[] = $deadline;
    $paramTypes .= "s"; // 's' untuk string
}

$whereSql = implode(' AND ', $whereClauses);
$query = "SELECT * FROM todos WHERE $whereSql ORDER BY deadline ASC";
