...
if (isset($_GET['undo'])) {
    ...
    $stmt = $conn->prepare("UPDATE todos SET status = 'pending' WHERE id = ? AND user_id = ?");
    $stmt->bind_param("ii", $task_id, $user_id);
    $stmt->execute();
    $stmt->close();
            ...
            <form method="POST" class="mb-4 px-4 py-4 rounded-4 shadow border border-light-subtle premium-form">
                <div class="row align-items-end g-3">
                    <div class="col-md-5">
                        ...
                        <input type="text" name="title" class="form-control form-control-sm glass-input rounded-5 shadow-sm" placeholder="Tulis tugasmu di sini..." required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label fw-semibold px-3">Deadline</label>
                        <input type="date" name="deadline" class="form-control form-control-sm glass-input rounded-5 shadow-sm" required>
                    </div>
