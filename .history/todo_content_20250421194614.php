...
$sql = "SELECT * FROM todos WHERE title LIKE ? AND user_id = ?";
$stmt = $conn->prepare($sql);  // Menggunakan prepared statement untuk keamanan
$stmt->bind_param("si", $searchParam, $userId);  // 'si' artinya string dan integer
$searchParam = "%" . $search . "%";  // Menambahkan wildcard untuk LIKE
